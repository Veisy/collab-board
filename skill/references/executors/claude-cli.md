...
process name) → classify a limit (signatures below; retry-vs-schedule for a matched signature
is decided **only** by the shared auto-resume rules in `../adapters.md`, including ambiguous
no-reset handling) → reconcile partial writes per the
